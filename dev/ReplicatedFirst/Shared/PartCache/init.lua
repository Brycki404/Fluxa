...
function PartCacheStatic.new(template: BasePart, numPrecreatedParts: number?, currentCacheParent: Instance?, minimumSize: number?): PartCache
	local newMinimumSize: number = minimumSize or 10	
	...
	assertwarn(newNumPrecreatedParts < newMinimumSize, "PrecreatedParts is < MinimumSize! This may have adverse effects when initially using the cache.")
...
function PartCacheStatic:Shrink(numParts: number)
	assert(getmetatable(self) == PartCacheStatic, ERR_NOT_INSTANCE:format("Shrink", "PartCache.new"))
	assert(typeof(numParts) == "number", "Shrink amount must be a number")

	-- Optional: allow user to define a minimum size
	local minimum = self.MinimumSize or 0

	-- How many parts CAN we remove?
	local openCount = #self.Open
	local totalCount = openCount + #self.InUse

	-- Never shrink below minimum
	local maxRemovable = math.max(0, totalCount - minimum)
	local toRemove = math.min(numParts, openCount, maxRemovable)

	if toRemove <= 0 then
		warn("Shrink() called but no removable parts available (cache at minimum or all parts in use).")
		return 0
	end

	for i = 1, toRemove do
		local part = table.remove(self.Open) -- removes from end
		if part then
			part:Destroy()
		end
	end
